...
class PSCLoss(nn.Module):
    def __init__(self, temp=0.1, alpha=0.2):
    ...
    def forward(self, feat, label, prototypes, probs, sample_per_class, discriminative=False, balanced=False):
        ...
        if balanced:
            ...
            # alpha = 1/5
            balanced_weight = balanced_negative_weight - balanced_GT_weight.unsqueeze(1)
            sign = torch.where(balanced_weight >= 0, 1, -1) # record sign
            balanced_weight = torch.where(balanced_weight >= 0, balanced_weight, -balanced_weight)  # turn to positive
            balanced_weight = torch.pow(balanced_weight, self.alpha) * sign  # multiply sign
        ...
        if balanced:
            negative_logits = torch.sum(torch.exp(logits + balanced_weight), dim=1)
        else:
...
def create_loss(temp=0.1, alpha=0.2):
    print('Loading PSCLoss.')
    return PSCLoss(temp, alpha)
